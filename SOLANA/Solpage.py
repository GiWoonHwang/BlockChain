...
def Solpage(request):
    try:
        ...
        if timeBtn == '' : # 특정 날짜를 입력하지 않으면 == 입금/출금/전체 내역만 보기 원한다면 사실 지워도 되지만 eden님과 상의
          ...
          if pandoWall == 'in':
            print('in')
            timeValue = EthScan.objects.filter(~Q(by = '0xa155ABc1DA12012702E32F29eE3d91472866eaD8'), lamport = 5000 ).order_by('-timeStamp')
            timeValueCount = EthScan.objects.filter(~Q(by = '0xa155ABc1DA12012702E32F29eE3d91472866eaD8'), lamport = 5000 ).count()
            timeValue = serializers.serialize('json', timeValue)
          # 출금 페이지
          elif pandoWall == 'out':
            print('out')
            timeValue = EthScan.objects.filter(~Q(to = '0xa155ABc1DA12012702E32F29eE3d91472866eaD8'),  lamport = 5000).order_by('-timeStamp')
            timeValueCount = EthScan.objects.filter(~Q(to = '0xa155ABc1DA12012702E32F29eE3d91472866eaD8'), lamport = 5000).count()
            timeValue = serializers.serialize('json', timeValue)
          # 전체 페이지
          elif pandoWall == 'all':
            print('all')
